...
async function getOpenProjects() {
    try {
        const response = await axios.get(`${process.env.REACT_APP_BACKEND}/lbd/public`);
        const data = response.data;
        return data;
    }
    catch (error) {
...
async function getUserProjects(token) {
    try {
        if (token) {
            ...
            const response = await axios.get(`${process.env.REACT_APP_BACKEND}/lbd`, config);
            const data = response.data;
            return data;
        }
        else {
...
async function createProject(project, token) {
    try {
        if (token) {
            ...
            const response = await axios.post(`${process.env.REACT_APP_BACKEND}/lbd`, project, config);
            const data = response.data;
            return data;
        }
        else {
 ...
 * @returns {Promise<Project>}
 */
async function getOneProject(project, token) {
    try {
 ...
 * @returns {Promise<void>}
 */
async function deleteProject(project, token) {
    try {
        const url = modifyProjectUrl(project);
        if (token) {
            const config = {
 ...
 * @returns {Promise<void>}
 */
async function deleteResource(url, token) {
    try {
        url = modifyUrl(url);
        let response;
        if (token) {
 ...
 */
async function uploadDocument(props, project, token) {
    try {
 ...
 */
async function getDocument(url, token) {
    try {
 ...
 * @returns {Promise<Metadata>}
 */
async function getDocumentMetadata(url, token) {
    try {
 ...
 * @returns {Promise<Metadata>}
 */
async function uploadGraph(props, project, token) {
    try {
        const baseUrl = modifyProjectUrl(project);
 ...
 */
async function getGraph(url, token) {
    try {
        const fullUrl = new URL(url);
        const realUrl = url.replace(`${fullUrl.protocol}//${fullUrl.host}`, `${process.env.REACT_APP_BACKEND}`);
        let response;
 ...
 * @returns {Promise<Metadata>}
 */
async function getGraphMetadata(url, token) {
    try {
        if (!url.endsWith(".meta")) {
            url = url.concat('.meta');
        }
 ...
 * @returns {Promise<void>}
 */
async function deleteGraph(url, token) {
    try {
        await deleteResource(url, token);
    }
    catch (error) {
...
async function updateProject(project, query, token) {
    try {
        let url = modifyProjectUrl(project);
 ...
 * @returns {Promise<void>}
 */
async function updateGraph(url, query, token) {
    try {
